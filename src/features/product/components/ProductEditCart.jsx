...
// Default values shownconst fetcher = (...args) => fetch(...args).then((res) => res.json());
const ProductEditCart = () => {
  const navigate = useNavigate();
  const {id} = useParams();
  
  const {data,isLoading,error} = useSWR(import.meta.env.VITE_API_URL + `/products/${id}`,fetchProducts)
...
{isLoading ? <div className="animate-pulse">
 ...
 : <form onSubmit={handleSubmit(handleEditProduct)}>
        ...
        <button disabled={isSubmitting} className="justify-center disabled:pointer-events-none disabled:opacity-80 inline-flex gap-2 items-center text-white mt-1 bg-blue-500 hover:bg-blue-700 font-bold py-2 px-4 rounded">
